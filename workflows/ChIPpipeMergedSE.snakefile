...
rule fastqc:
	...
	shell:
		...
		fastqc -o {params.dir} {input.R1} 1> {log.out} 2> {log.err};
...
rule multiqc:
	input:
		[expand("output/QC/{sampleName}_{read}_fastqc.{ext}", sampleName=key, read=['R1'], ext=['zip', 'html']) for key in samples['sn']],
		[expand("output/trim/{sampleName}_{read}_{ext}", sampleName=key, read=['R1'], ext=['trimming_report.txt', 'trimmed.fastq.gz']) for key in samples['sn']],
